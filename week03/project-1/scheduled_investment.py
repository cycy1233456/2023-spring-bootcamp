...
def calculate_scheduled_investment_fixed_cost_with_sell(data: pd.DataFrame,
    ...
    for i in range(1, len(data)):
        ...
        for i in range(1, len(data)):
            open_price = data.iloc[i]['OPEN']
            date = data.iloc[i]['DATES']
            if assets[i - 1] >= min_asset and days >= min_days and assets[i - 1] >= sell_point * cost[i - 1]:
                # sell
...
# -- TODO: Part 5 (Bonus)
def print_all_annual_returns():
    """
    Calculate and print the annualized returns for the three different investment strategies.
    """
    print("Investment Return 1: ", round(export_result() * 100, 2), "%")
    print("Investment Return 2: ", round(get_annual_return_fixed_cost() * 100, 2), "%")
    print("Investment Return 3: ", round(get_annual_return_fixed_cost_with_sell() * 100, 2), "%")
